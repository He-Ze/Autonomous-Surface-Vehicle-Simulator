 ...
 */
public class DoubleTextArea extends TextArea {
   ...
   * @param minValue
     * @param maxValue
     */
  public DoubleTextArea(Vector2f position, Vector2f size, String fontName, int fontStyle, double minValue,
                        double maxValue) {
   ...
   */
  protected double parseCurrentText(String text) {
    double parsedInteger = 0;
    boolean validNumber = false;
    try {
      // Try to parse the text as an double.
      parsedInteger = Double.parseDouble(text);
      validNumber = true;
  ...
  public void setText(String newText) {
    if(hasLoaded){
      double newCurrent = parseCurrentText(newText);
   ...
   * @param newCurrent
   */
  public void setValue(double newCurrent) {
    if (newCurrent != this.current) {

      //Limit new values.
      newCurrent = Math.max(newCurrent, minimum);
      newCurrent = Math.min(newCurrent, maximum);
